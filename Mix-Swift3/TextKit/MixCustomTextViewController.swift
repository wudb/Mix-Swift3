...
class MixCustomTextViewController: UIViewController, UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        // 高亮
        self.textStorage?.addAttribute(NSForegroundColorAttributeName, value: UIColor.red, range: NSMakeRange(0, 5))
        ...
        // 字体
        self.textStorage?.addAttribute(NSFontAttributeName, value: UIFont.boldSystemFont(ofSize: 50), range: NSMakeRange(220, 1))
        
        // 背景色
        self.textStorage?.addAttribute(NSBackgroundColorAttributeName, value: UIColor.yellow, range: NSMakeRange(30, 10))
        ...
        self.imageView?.center = CGPoint(x: self.view.bounds.size.width / 2, y: self.imageView!.frame.size.height / 2 + 200)
    ...
    func updateExclusionPaths() {
        // 计算图片所占范围
        var imageRect = self.textView?.convert(self.imageView!.frame, from: self.view)
    ...
    func imagePan(pan: UIPanGestureRecognizer) {
        if pan.state == .began {
            self.panOffset = pan.location(in: self.imageView!)
        }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        self.imageView?.center = CGPoint(x: self.imageView!.center.x, y: self.imageCenterY - (scrollView.contentOffset.y + navigationBarHeight))
    }
